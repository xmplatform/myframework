...
public abstract class SqlExecutor {
    /** max size */
    public static final int MAX_SIZE = Context.app("SQL_MAX_SIZE", 50000);
    ...
    public Bean queryById(String sql, List<Object> values) {
        List<Bean> rtnList = query(Transaction.getConn(), sql, values);
        if (!rtnList.isEmpty()) {
                    ...
                    switch (columnBean.getInt("TYPE")) {
                        ...
                        case Types.NUMERIC:
                            ...
                            if (value == null) {
	                            ...
	                            if (((BigDecimal)value).compareTo(new BigDecimal(Integer.MAX_VALUE)) > 0) {
    ...
    public int count(Connection conn, String sql, List<Object> values) {
        ...
        if (sql.matches(pn)) {
            sql = "select count(*) COUNT_ from (" + sql + ")";
        } else {
    ...
    public int executeBatchBean(Connection conn, String beanSql, List<Bean> beanList) {
        ...
        try {
            ...
            stmt = conn.prepareStatement(psql);
            long startTime = System.currentTimeMillis(); // 起始时间
            int batchSize = Context.getSyConf("SY_SQL_BATCH_SIZE", 10000);
    ...
    protected boolean isLogSql(long sqlTime, int sqlCount) {
        ...
        if (log.isDebugEnabled()) { //调试模式自动记录sql
            ...
            if (!Context.isEmpytyThread(THREAD.LOGDEBUG)) {
                if (!(Boolean) Context.getThread(THREAD.LOGDEBUG)) {
                    logSql = false;
                }
            }
        } else { //非调试模式
            if (Context.appInt(LOG_SQL_TIME) > 0) { 
                if (sqlTime > Context.appInt(LOG_SQL_TIME)) { //超时记录sql
                    logSql = true;
                }
            }
            if (Context.appInt(LOG_SQL_SIZE) > 0) {
                if (sqlCount > Context.appInt(LOG_SQL_SIZE)) { //超长记录sql
                    logSql = true;
                }
            }
        }
