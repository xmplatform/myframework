...
public class Transaction {
	...
	public static void begin(String dsName) {
		LinkedList<Bean> tranList = (LinkedList<Bean>) Context.getThread(THREAD.TRANSLIST);
		if (tranList == null) {
			tranList = new LinkedList<Bean>();
			Context.setThread(THREAD.TRANSLIST, tranList);
		}
		Bean dsBean = Context.getDSBean(dsName);
	...
	public static void commit() {
		Bean tranBean = ((LinkedList<Bean>) Context.getThread(THREAD.TRANSLIST)).getFirst();
		try {
	...
	public static DB_TYPE getDBType() {
        ...
        if (tranList != null && !tranList.isEmpty()) {
			...
			if (tranBean != null) {
				...
				type = (DB_TYPE) Context.getDSBean().get(DS.DB_TYPE);
			}
		} else {
			type = (DB_TYPE) Context.getDSBean().get(DS.DB_TYPE);
		}
