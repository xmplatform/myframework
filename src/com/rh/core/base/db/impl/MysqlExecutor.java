...
public class MysqlExecutor extends SqlExecutor {
    ...
    public void queryCall(Connection conn, String sql, List<Object> values, IRowHandler rh, QueryCallback qc) {
        ...
        try {
            ...
            for (int i = 1; i <= columnCount; i++) {
                ...
                columnBean.set("NAME", rsmd.getColumnLabel(i));
    ...
    public TableBean getDBTable(String tableCode) {
        ...
        try {
            ...
            if (conn == null) {
                conn = Context.getConn();  // 获取缺省数据源的连接
                ownConn = true;
            }
            dbmd = conn.getMetaData();
            String catelog = Context.getDSBean(Transaction.getDsName()).getStr(DS.SCHEMA);
            ...
            if (trs.next()) {
                ...
                while (irs.next()) { // 获取字段定义
                    ...
                    if (type.equals("NUMBER") || type.equals("NUMBERIC")) { // 数字类型
                        fieldBean.set("ITEM_FIELD_TYPE", ServConstant.ITEM_FIELD_TYPE_NUM);
                        ...
                        fieldBean.set("ITEM_FIELD_TYPE", ServConstant.ITEM_FIELD_TYPE_BIGTEXT);
                        length = "99999999";
                    } else if (type.equals("TIMESTAMP")) {
                        fieldBean.set("ITEM_FIELD_TYPE", ServConstant.ITEM_FIELD_TYPE_TIME);
                        length = "26";
                    } else {
                        fieldBean.set("ITEM_FIELD_TYPE", ServConstant.ITEM_FIELD_TYPE_STR);
                    }
                    ...
                    fieldBean.set("ITEM_TYPE", ServConstant.ITEM_TYPE_TABLE); //表字段
