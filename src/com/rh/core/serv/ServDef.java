...
public class ServDef extends CommonServ {
    ...
    public OutBean copyOf(ParamBean paramBean) {
	    ...
	    ServUtils.clearServCache(servId);
        ...
        if (paramBean.getId().length() > 0) {
            ...
            for (String servId : servIds) {
                ...
                if (serv != null) {
	                ...
	                param.setId(servId).setServId(SERV_SY_SERV).setLinkFlag(true).set("$JSON_FLAG", false); //不删除文件、级联删除
	                delete(param);
    ...
    /**
     * 导入JsonBean格式的服务定义，如果已经存在，则直接覆盖当前服务定义（删除后导入） 会生成json文件
